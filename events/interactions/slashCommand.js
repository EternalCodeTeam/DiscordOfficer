...
module.exports = async (client, interaction) => {
    ...
    try {
        if (slashCommand.cooldown && cooldown.has(`slash-${slashCommand.name}${interaction.user.id}`)) {
            let againUseCooldown = ms(cooldown.get(`slash-${slashCommand.name}${interaction.user.id}`) - Date.now(), { long: true });

            return interaction.reply({
                content: `Musisz jeszcze poczekać ${againUseCooldown} zanim ponownie użyjesz tego polecenia!`
            });
        }
