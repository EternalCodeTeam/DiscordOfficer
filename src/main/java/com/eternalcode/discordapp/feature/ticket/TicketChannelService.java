...
public class TicketChannelService {
    ...
    private TextChannel createChannel(long userId, TicketConfig.TicketCategoryConfig category) {
        ...
        if (member == null) {
            throw new IllegalStateException("Cannot create ticket channel, member not found in guild: " + userId);
        }

        Role staffRole = this.config.staffRoleId != 0L ? guild.getRoleById(this.config.staffRoleId) : null;

        var channelAction = cat.createTextChannel(channelName)
        ...
        if (staffRole != null) {
            channelAction = channelAction.addPermissionOverride(staffRole,
                List.of(
        ...
        else if (this.config.staffRoleId != 0L) {
            LOGGER.warn(
                "Staff role not found with ID: {}. Creating ticket without staff role override.",
