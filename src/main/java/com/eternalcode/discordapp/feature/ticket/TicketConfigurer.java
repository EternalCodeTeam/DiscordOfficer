...
public class TicketConfigurer {
    ...
    public void initialize() {
        try {
            ...
            // Manually register ticket command as a safe fallback in case CommandClientBuilder misses late additions.
            ...
            Guild guild = this.jda.getGuildById(this.appConfig.guildId);
            if (guild == null) {
                LOGGER.warn("Skipping manual ticket command registration, guild not found: {}", this.appConfig.guildId);
            }
            else {
                // upsertCommand updates/creates a single command without replacing all guild commands.
                guild.upsertCommand(ticketCommandData).queue(
                    success -> LOGGER.info("Ticket command upserted via JDA API successfully"),
                    failure -> LOGGER.error("Failed to upsert ticket command via JDA API", failure)
                );
            }
